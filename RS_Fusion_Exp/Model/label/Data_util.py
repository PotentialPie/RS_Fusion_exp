...
def Crop(image, seg_label, size=224, stride=20):
    """
    根据图像和语义分割图，进行crop
    :param image: 给定语义分割图
    :param seg_label: 给定语义分割图
    ...
    start_col = 0
    '''
    while start_row + size < seg_label.shape[1]:
        end_row = start_row + size
        while start_col + size < seg_label.shape[2]:
    ...
    '''
    while True:
        if start_row + size >= seg_label.shape[1]:
            end_row = seg_label.shape[1]
            start_row = end_row - size
            while True:
                if start_col + size >= seg_label.shape[2]:
                    end_col = seg_label.shape[2]
                    start_col = end_col - size
                    # print(
                    #    str(count) + ' ' + str(start_row) + ' ' + str(end_row) + ' ' + str(start_col) + ' ' + str(end_col))
                    img_feature = image[:, start_row:end_row, start_col:end_col]
                    img_label = seg_label[:, start_row:end_row, start_col:end_col]
                    image_list.append(img_feature)
                    label_list.append(img_label)
