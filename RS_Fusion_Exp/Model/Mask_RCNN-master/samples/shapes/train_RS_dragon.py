...
class RSDataset(utils.Dataset):
    ...
    def load_RS(self, count, height, width):
        ...
        for i in count:
            subimg_path = '/disk4/Zhangyunfeng/RS/RS_Fusion_Exp/Data/labels/image_%s.npz' % (i)
...
for i in range(15, 20):
    ...
    for ind in range(20):
        train_set.add(ind + 1)
    train_set.remove(i+1)
    train_list = list(train_set)
    dataset_train.load_RS(train_list, config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1])
    ...
    dataset_val.load_RS([i+1], config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1])
    dataset_val.prepare()
    print('验证数据准备完毕')
